...
export default function ChatListUser() {
    ...
    useEffect(() => {
        // Subscribing to the private channel for new messages
        const newMessageChannel = Echo.private(`message.${auth.user.uuid}`);

        newMessageChannel.listen('NewMessageEvent', (event) => {
            const sender = event.message.sender;

            toast(`You have a new message from ${sender.name}`, {
                duration: 4000,
                position: 'top-right',
                style: {
                    background: '#333',
        ...
        return () => {
            newMessageChannel.stopListening('NewMessageEvent');
        };
    }, [auth.user.uuid]);
